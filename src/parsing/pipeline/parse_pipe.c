...
/*   Updated: 2023/09/04 13:59:37 by  mchenava        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	process_block(t_sh_context *shx, t_block **blocks, t_str *splited, t_uint *i, t_uint *start)
{
	t_uint	status;
...
int	parse_pipeline(t_sh_context *shx, t_block **blocks, t_str *splited)
{
	...
	{
		start = i;
		status = process_block(shx, blocks, splited, &i, &start);
		if (status != 0)
			return (status);
		*blocks = (*blocks)->next;
	}
